...
class Client:
    ...
    def subscr(self,callback,topic):
        sub = Subscriber(broker,self,callback,topic)
        self.subscribed.append(sub)
        return sub
    
    def topic_subscriber(self,topic):
        for s in subscribed:
            if s.get_filter() == topic:
                return s
            
        return None
        
    ##  pubish a message to a topic
